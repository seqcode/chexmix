...
/**
 * ProgressiveProfileAlignment: performs progressive alignment of the ChIP-exo footprints
 * 
 * input : experiment
 * 		   set of points
 * output: an aligned profile
 * 		   a guide tree
 *
 * @author naomi
 *
 */
public class ProgressiveProfileAlignment {
	...
	protected List<Map<Integer, Profile>> condIndex2profileMap;
	protected PriorityQueue<Profile> priorityQueue;
	protected int win;
	...
	public ProgressiveProfileAlignment(List<StrandedPoint> points, ExperimentManager eMan, int win, String filename){
		this.points = points;
		exptMan = eMan;
		this.win = win;
		...
		condIndex2profileMap = new ArrayList<Map<Integer,Profile>>();
	}
	
	public void initPriorityQueue(){
		for(ExperimentCondition cond : exptMan.getConditions()){
			Map<StrandedPoint, Integer> points2index = new HashMap<StrandedPoint, Integer>();
			Map<Integer, StrandedPoint> index2points = new HashMap<Integer, StrandedPoint>();
			Map<Integer, Profile> index2profile = new HashMap<Integer, Profile>();
			//Iterate through points to create index and profiles
			int p=0;
			for (StrandedPoint pt : points){
				points2index.put(pt, p);
				index2points.put(p, pt);
				p++;
			}
			List<Profile> queueElem = new ArrayList<Profile>();
			// Create all pair wise comparisons
			for (int a=0; a < points.size(); a++){
				for (int b=0; b < points.size(); b++){
					if (a != b){
						// Get number of tags 
						double[] watsonA = maker.getPointWatson(index2points.get(a), cond);
						double[] crickA = maker.getPointCrick(index2points.get(a), cond);
						double[] watsonB = maker.getPointWatson(index2points.get(b), cond);
						double[] crickB = maker.getPointWatson(index2points.get(b), cond);						
						...
						Profile mergedProfile = new Profile(corr.getWatsonProfile(), corr.getCrickProfile(), p);
						p++;
						// Store index
						mergedProfile.setPairProfile(a, b, corr.getMaxScore());
						mergedProfile.setProfile(true);
						mergedProfile.setOffset(corr.getOffset());
						mergedProfile.setReverse(corr.isReverse());
						queueElem.add(mergedProfile);

						index2profile.put(p, mergedProfile);
					}
				}
			}
			// constructor for a priority queue
			priorityQueue = new PriorityQueue(queueElem); 	
			//print the contents
			System.out.println(priorityQueue.toArray());
		}
	}
	
	public void add2PriorityQueue(Profile profile){
	...
	public void execute(boolean useNW){
		...
		Profile currProfile = priorityQueue.poll();
		
		// Construct an initial tree by taking the most similar profile
		DefaultMutableTreeNode initNode = new DefaultMutableTreeNode(currProfile);
		int indexA = currProfile.getIndexA();		
		int indexB = currProfile.getIndexB();
		
		// This needs work from here !
...
//		System.out.println("offset "+offset);
	...
	public static void main(String[] args){
		...
		if(args.length==0){
			...
			ProgressiveProfileAlignment alignment = new ProgressiveProfileAlignment(pts, manager, w, filename);			
