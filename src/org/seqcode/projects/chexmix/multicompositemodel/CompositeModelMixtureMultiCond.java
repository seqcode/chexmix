 ...
 * @version	%I%, %G%
 */
public class CompositeModelMixtureMultiCond {
	...
	protected ProteinDNAInteractionModelMultiCond model; //The model to train
	protected List<CompositeModelSiteAssignmentMultiCond> siteAssignments;
	protected CompositeModelEMMultiCond EMtrainer; //Training method
	...
	public CompositeModelMixtureMultiCond(CompositeTagDistribution tagDist, CompositeTagDistribution ctrlDist, GenomeConfig gcon, ExptConfig econ, ChExMixConfig ccon, ExperimentManager eMan){
		gconfig = gcon;
		econfig = econ;
		config = ccon;
		manager = eMan;
		...
		EMtrainer = new CompositeModelEMMultiCond(compositeDistrib, config, manager);

		initializeTagDistributions();
		
		model = new ProteinDNAInteractionModelMultiCond(config, compositeDistrib, initXLDistrib, initCSDistrib, initBackDistrib, initNoisePi);
		siteAssignments = new ArrayList<CompositeModelSiteAssignmentMultiCond>();
	}
	
	//Accessors
	public ProteinDNAInteractionModelMultiCond getModel(){return model;}
	public void setModel(ProteinDNAInteractionModelMultiCond mod){model=mod; trainedModel=true;} //Useful if not training model, but still want to use ML on a saved model
	public List<CompositeModelSiteAssignmentMultiCond> getSiteAssignments(){return siteAssignments;}
		...
		try{
			...
			for(trainingRound=0; trainingRound<config.getMaxModelUpdateRounds() && !converged; trainingRound++){
				...
				for (ExperimentCondition cond : manager.getConditions()){
					int c=cond.getIndex();
					System.out.println("TrainingRound: "+trainingRound+"\tCondition: "+cond.getName()+":\n"+model.toString(c)+"\n\tModelKL="+kl[trainingRound][c]);
					
					//Check for convergence
					if(trainingRound>=config.getMinModelUpdateRounds()-1 && kl[trainingRound][c]<config.getModelConvergenceKL())
						converged=true; 
				}
			}
			if(config.getPlotEM())
	...
	public void assignML(boolean updatePi){
		try{
			if(trainedModel){
				...
				//Run ML
				CompositeModelMLAssignMultiCond MLassigner = new CompositeModelMLAssignMultiCond(compositeDistrib, model, config, manager);
				for(int s=0; s<compositeDistrib.getPoints().size(); s++){
					
					CompositeModelSiteAssignmentMultiCond currAssignment = MLassigner.assignToPointFromComposite(s, updatePi);
    ...
    public double[] updateTagDistributions(){
    	double[] logKL = new double[manager.getNumConditions()];
    	for(ExperimentCondition cond : manager.getConditions())
    		logKL[cond.getIndex()] = 0;
    	...
    	for(ExperimentCondition cond : manager.getConditions()){
    		...
    		//Calc KL
    		logKL[c] += StatUtil.log_KL_Divergence(oldCSModelW, newCSModelW);
    		logKL[c] += StatUtil.log_KL_Divergence(oldCSModelC, newCSModelC);
...
//    					}
    				...
    				//Calc KL
    				logKL[c] += StatUtil.log_KL_Divergence(oldXLModelW, newXLModelW);
    				logKL[c] += StatUtil.log_KL_Divergence(oldXLModelC, newXLModelC);
    			}
    		}
    	}
	...
	public void printPerSiteComponentResponsibilitiesToFile(ExperimentCondition cond, String filename){
		try {
			...
			for(CompositeModelComponent nz : model.getNonZeroComponents(c)){
				int pos = nz.getPosition() - model.getCenterOffset();
				String compLabel = nz.getLabel()+","+pos;
				fout.write("\t"+compLabel);
			}fout.write("\n");
			
			for(CompositeModelSiteAssignmentMultiCond cmsa : siteAssignments){
	...
	public void printComponentProfilesToFile(ExperimentCondition cond, String filename){
		try {
			...
			String out="";
			for (CompositeModelComponent comp :  model.getAllComponents(cond.getIndex())){
	...
	public void saveCompositePlots(){
		try {
			...
			for(ExperimentCondition cond : manager.getConditions()){
				...
				TagProfile back = new TagProfile(model.getBackgroundComponent(c).getTagProfile(true), 
						model.getBackgroundComponent(c).getTagProfile(false), 
						compositeDistrib.getCenterOffset());
				...
				String backImageFileName = config.getOutputImagesDir()+File.separator+config.getOutBase()+"_backCompResponsibilities."+cond.getName()+".png";
				...
				String backZoomImageFileName = config.getOutputImagesDir()+File.separator+config.getOutBase()+"_backCompResponsibilitiesZoom."+cond.getName()+".png";
				backPainter.saveImage(new File(backZoomImageFileName), 1060, 500, true);
			
				//CS component responsibilities 
				TagProfile cs = new TagProfile(model.getCSComponent(c).getTagProfile(true), 
						model.getCSComponent(c).getTagProfile(false), 
						compositeDistrib.getCenterOffset());
				...
				String csImageFileName = config.getOutputImagesDir()+File.separator+config.getOutBase()+"_csCompResponsibilities."+cond.getName()+".png";
				...
				String csZoomImageFileName = config.getOutputImagesDir()+File.separator+config.getOutBase()+"_csCompResponsibilitiesZoom."+cond.getName()+".png";
				...
				for(CompositeModelComponent xlComp : model.getXLComponents(c)){
					if(xlComp.isNonZero() && xlComp.getTagProfile(true) !=null){
						...
						String xlImageFileName = config.getOutputImagesDir()+File.separator+config.getOutBase()+"_XL"+pos+"Responsibilities."+cond.getName()+".png";
						...
						String xlZoomImageFileName  = config.getOutputImagesDir()+File.separator+config.getOutBase()+"_XL"+pos+"ResponsibilitiesZoom."+cond.getName()+".png";
